...
{
    if (!count($listOfArt) or !count($listOfCat)) return '';

    $number_Of_Categories = [];
    $result = [];

    foreach ($listOfCat as $category) {
        $number_Of_Categories[$category] = 0;
        foreach ($listOfArt as $art) {
            if ($category === $art[0]) {
                $number_Of_Categories[$art[0]] += preg_filter('#[A-Z ]#', '', $art);
            }
        }
    }
    foreach ($number_Of_Categories as $category => $number) {
